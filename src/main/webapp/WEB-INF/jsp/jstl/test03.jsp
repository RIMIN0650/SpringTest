...
<body>
	...
	<table class="table text-center">
		...
		<tbody>
			...
			<c:forEach var="bill" items="${cardBillList }">
			<tr>
				<td>${bill.store }</td>
		...
		<%-- 				
				<c:set var="dateee" value="${bill.date }"/>
				<fmt:formatDate value="${dateee }" pattern="yyyy-MM-dd" var="date"/>
				<fmt:parseDate value="${date }" pattern="yyyy년MM월dd일 " var="dateResult"/>
				<td>${dateResult }</td>
		 --%>		
